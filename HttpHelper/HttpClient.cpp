 ...
 * 返回服务器的response
 */
Response* HttpClient::httpRequest(char* url, PostParameter* postParams, int count, bool authenticated, char* httpMethod)
{
    Response* response = new Response();

    // 注册收到response header的事件响应
    connect(this->http, SIGNAL(responseHeaderRecieved(const QHttpResponseHeader&)),
    ...
    if (this->host != NULL) {
        ...
        if (httpMethod == HTTP_METHOD_POST) {
            // 生成post参数列表并发送post请求
            QString paras = PostParameter::generatePostParameter(postParams, count);
